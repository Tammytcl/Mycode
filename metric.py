...
class DiceLoss(nn.Module):
    ...
    def _one_hot_encoder(self, input_tensor):
        ...
        if input_tensor.dim() != 4:
            raise ValueError(f'Expected 4D tensor for input, got {input_tensor.dim()}D tensor')

        shape = input_tensor.shape
        one_hot = torch.zeros((shape[0], self.n_classes, shape[2], shape[3]), device=input_tensor.device)
        for i in range(self.n_classes):
            one_hot[:, i, :, :] = (input_tensor == i).float()
        return one_hot
    ...
    def _dice_loss(self, score, target):
        ...
        y_sum = torch.sum(target, dim=[1, 2, 3])
        z_sum = torch.sum(score, dim=[1, 2, 3])
    ...
    def forward(self, inputs, target, weight=None, softmax=False):
        ...
        # 确保输入和目标具有相同的形状
        assert inputs.size() == target.size(), f'预测尺寸 {inputs.size()} 与目标尺寸 {target.size()} 不匹配'
...
class GeneralizedSoftDiceLoss(nn.Module):
    ...
    def __init__(self,
        ...
        self.weight = None if weight is None else torch.tensor(weight, dtype=torch.float)
    ...
    def forward(self, logits, label):
        ...
        # Convert label to long tensor for scatter
        label = label.long()
